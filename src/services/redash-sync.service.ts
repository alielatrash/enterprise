...
export async function syncActualRequests(): Promise<number> {
  ...
  try {
    ...
    for (const record of records) {
      // Expected columns: id, shipper_name, pickup_city, dropoff_city, request_date, truck_type, loads_requested, loads_fulfilled
    ...
    console.error('Actual requests sync error:', errorMessage)
...
export async function syncFleetCompletions(): Promise<number> {
  ...
  try {
    ...
    let count = 0
    for (const record of records) {
      // Expected columns: id, supplier_name, pickup_city, dropoff_city, completion_date, truck_type, loads_completed
    ...
    const errorMessage = error instanceof Error ? error.message : 'Unknown error'
    console.error('Fleet completions sync error:', errorMessage)
